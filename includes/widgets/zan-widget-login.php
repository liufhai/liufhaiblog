...
class Zan_Login extends WP_Widget {
  ...
  function Zan_Login() {
    $widget_options = array(
          'name'        => '前台登录组件（LiufhaiBlog）',
          'description' => 'LiufhaiBlog 前台登录组件'
    );
  ...
  function widget( $args, $instance ) {  
    ...
    $title_login = '';
    $title_logout = '';
    @$title_login = $instance['title_login'] ? $instance['title_login'] : '<i class="fa fa-sign-in"></i>  网站登录';
    @$title_logout = $instance['title_logout'] ? $instance['title_logout'] : '<i class="fa fa-sign-out"></i>  欢迎主银回来~~~么么哒';
    echo $before_widget;
    ?>
    ...
    <div class="panel panel-zan">
      ...
      <form class="login-form clearfix" action="<?php echo get_option( 'home' ); ?>/wp-login.php" method="post">
        ...
        <a href="<?php echo home_url().'/wp-login.php?action=register'?>" class="btn btn-zan-solid-ip pull-right">注册</a>
      </form>
    </div>
    <?php } else { ?>
    <div class="panel panel-zan">
      ...
      <div class="login-panel text-center">
          ...
          <a class="user-name" href="<?php echo home_url().'/wp-admin/index.php';?>"><?php echo $current_user->user_login; ?></a>
          <a class="btn btn-zan-solid-pw" href="<?php echo wp_logout_url( get_bloginfo( 'url' ) ); ?>" title="退出登录">退出登录</a>
      </div>
    </div>
    <?php }?>
    <?php
